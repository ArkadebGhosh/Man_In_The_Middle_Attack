...
def Client(serverhostname):
    ...
    clientsock.sendall("chat_hello".encode('UTF-8'))
    msg = clientsock.recv(Buffer)
    print("Message from Bob: ", msg.decode('UTF-8'))

    # Alice now sends chat_STARTTLS to establish a secure TLS pipe
    clientsock.sendall("chat_STARTTLS".encode('UTF-8'))
    print("chat_STARTTLS sent to Bob")
    ...
    if msg == 'chat_STARTTLS_ACK':
        ...
        while(True):
            ...
            secureClientSocket.sendall(msg.encode('UTF-8'))
            if msg == 'chat_close':
                secureClientSocket.close()
                clientsock.close()
                print("Closing TLSv1.3 and TCP Connection")
    ...
    elif msg == 'chat_STARTTLS_NOTSUPPORTED':
